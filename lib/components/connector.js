...
function getSession(self, socket) {
    ...
    session = self.session.service.create(sid, app.getServerId(), socket);
...
function bindEvents(self, socket) {
    console.log(self.connection.service);
    if (self.connection) {
        self.connection.service.increaseConnectionCount();
        const statisticInfo = self.connection.service.getStatisticsInfo();
    ...
    socket.on('disconnect', () => {
        ...
        closed = true;
        if (self.connection) {
            self.connection.service.decreaseConnectionCount(session.uid);
        }
    });
