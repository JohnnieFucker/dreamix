...
class Component {
     ...
     */
    afterStart(cb) {
        logger.debug('server [%s] afterStart and rpc client proxies = %j', this.app.getServerId(), this.client.proxies);
    ...
    addServers(servers) {
        ...
        logger.debug('server [%s] addServers %j this.app.rpc = %j this.app.sysrpc =%j', this.app.getServerId(), servers, this.app.rpc, this.app.sysrpc);
    }
