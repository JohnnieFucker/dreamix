...
function initFilter(isGlobal, app) {
    const service = new FilterService(app.serverId);
    let befores;
    let afters;
    ...
    logger.debug('init server[%s] filter:  befores %j afters %j', app.serverId, befores, afters);
    if (befores) {
...
function doForward(app, msg, session, routeRecord, cb) {
    ...
    try {
        logger.debug('doForward msg= %j,routeRecord = %j', msg, routeRecord);
        console.log(app.sysrpc[routeRecord.serverType].msgRemote);
        logger.debug('doForward msgRemote = %j', app.sysrpc[routeRecord.serverType].msgRemote);
        ...
        app.sysrpc[routeRecord.serverType].msgRemote.forwardMessage(
            ...
            (err, resp, opts) => {
                ...
                finished = true;
                logger.debug('doForward %s,resp = %j', finished, resp);
                utils.invokeCallback(cb, err, resp, opts);
            }
        );
    } catch (err) {
        logger.error(`fail 1 to forward message:${err.stack}`);
        if (!finished) {
...
class Server {
    ...
    globalHandle(msg, session, cb) {
        ...
        logger.debug('routeRecord = %j', routeRecord);
        ...
        const dispatch = (err, resp, opts) => {
            ...
            if (self.app.getServerType() !== routeRecord.serverType) {
                logger.debug(`from ${self.app.getServerType()} forward msg to ${routeRecord.serverType}`);
                doForward(self.app, msg, session, routeRecord, (_err, _resp, _opts) => {
                    response(true, self, _err, msg, session, _resp, _opts, cb);
                });
            } else {
                logger.debug(`do handle msg on ${routeRecord.serverType}`);
