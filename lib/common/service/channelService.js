    ...
    if (!self.store) {
        ...
        loadAllFromStore(self, genKey(self), (err, list) => {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                self.channels[name] = new Channel(name, self);// eslint-disable-line
                load(list[i], name);
            }
            utils.invokeCallback(cb);
        });
 ...
 * component of dreamix and channel service would be accessed by `app.get('channelService')`.
 *
 * @class
 * @constructor
 */
class ChannelService {
    constructor(app, opts) {
        ...
        this.channelRemote = ChannelRemote(app);
    }
