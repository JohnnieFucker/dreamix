...
function rpcInvoke(app, sid, namespace, service, method, args, cb) {
...
class BackendSession {
    ...
    /**
     * update current session with the userInfo. It would push the userInfo to frontend
     * server and bind  userInfo to the frontend internal session.
     *
    ...
    updateUserInfo(userInfo, cb) {
        const self = this;
        this.__sessionService__.updateUserInfo(this.frontendId, this.id, userInfo, (err) => {
            if (!err) {
...
class BackendSessionService {
    ...
    /**
     * Bind the session with the specified user Info. It would finally invoke the
     * the sessionService.updateUserInfo in the cooperating frontend server.
     *
     * @param  {String}   frontendId cooperating frontend server id
     * @param  {Number}   sid        session id
     * @param  {Object}   userInfo
     ...
     * @api private
     */
    updateUserInfo(frontendId, sid, userInfo, cb) {
        const namespace = 'sys';
        const service = 'sessionRemote';
        const method = 'updateUserInfo';
        const args = [sid, userInfo];
        rpcInvoke(this.app, frontendId, namespace, service, method, args, cb);
    }
