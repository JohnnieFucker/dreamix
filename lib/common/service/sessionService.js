...
const logger = require('dreamix-logger').getLogger('pomelo', __filename);
...
class FrontendSession extends EventEmitter {
    ...
    bind(uid, cb) {
        ...
        this.__sessionService__.bind(this.id, uid, (err) => {
            logger.error('before bind uid %j', self.uid);
            logger.error('bind err %j', err);
            if (!err) {
                self.uid = uid;
            }
    ...
    updateUserInfo(userInfo, cb) {
        const self = this;
        this.__sessionService__.updateUserInfo(this.id, userInfo, (err) => {
            if (!err) {
...
class Session extends EventEmitter {
    ...
    */
    ...
    /**
     * update the session userInfo such as client_os,company_id.
     *
...
class SessionService {
    constructor(opts) {
        opts = opts || {};
        this.singleSession = opts.singleSession ? opts.singleSession : false;
     ...
     *
     * @param {Number} sid uniqe id for the internal session
    ...
    /**
     * update the session with a userInfo.
     *
     * @memberOf SessionService
     * @api private
     */
    updateUserInfo(sid, userInfo, cb) {
        const session = this.sessions[sid];
    ...
    sendMessageByUid4Mobile(uid, msg) {
        const sessions = this.uidMap[uid];
        if (!sessions) {
            logger.debug('fail to send message 4 mobile by uid for non-existing session. uid: %j',
                uid);
            return false;
        }
        for (let i = 0, l = sessions.length; i < l; i++) {
            // 检查session 如果是os不是手机客户端则不发送私秘消息
            if (sessions[i] && (sessions[i].get('os') === 'ios' || sessions[i].get('os') === 'android')) {
                send(this, sessions[i], msg);
            }
        }
        return true;
    ...
    forEachBindedSession(cb) {
        ...
        for (const uid in this.uidMap) {
            if (this.uidMap.hasOwnProperty(uid)) {
                sessions = this.uidMap[uid];
                for (let i = 0, l = sessions.length; i < l; i++) {
                    cb(sessions[i]);
                }
